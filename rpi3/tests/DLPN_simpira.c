...
int Deterministic_LPN_Simpira(uint8_t *input){
  simpira_perm( input );
  ...
  for (int i = 0; i < 16; i++)
  {
    for (int j = 0; j < 8; j++)
    {
      flat_binary_output[8*i+j] = (input[i] >> j)&1;
    }
  }
...
int main() {
    ...
    // LPN secret
    uint8_t s[INPUT_BYTELENGTH];
    for (int i=0; i < INPUT_BYTELENGTH; i++){
        s[i] = (21*i)%256;
    }
    ...
    {
      uint8_t sample[B*16] = {0}; 
      for (int j =0; j < B*16; j++)
      {
          sample[j] = data[(B*16)*sample_i + j] ^ s[j];
      }
      
      Deterministic_LPN_Simpira(sample);
    }
    printf("Warmup done\n");
...
// gcc tests/DLPN_simpira.c -Icommon/simpirav2 common/simpirav2/tinyAES/aes.c -mfpu=neon -O3 -o test && test
